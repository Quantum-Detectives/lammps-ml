...
double ipow_vdw(double b, int e){
    double r = 1.0;
    while (e>0){
        if (e & 1) r *= b;
        e >>= 1;
...
void matmul_vdw(double* __restrict__ a, const double* b, const double* c, int iM, int jM, int kM){
  for(int i=0; i<iM; i++)
    for(int k=0; k<kM; k++)
      for(int j=0;j<jM; j++)
...
PairPANNAvdw::PairPANNAvdw(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
  single_enable = 0;
}
...
void PairPANNAvdw::compute_gvect(int ind1, double **x, int* type,
  ...
  for(int n=0; n<nan-1; n++){
    for(int m=n+1; m<nan; m++){
      ...
      for(int Thi=0; Thi<par.ThetasN; Thi++){
        ...
        mod_norm[Thi] = ipow_vdw( 0.5 * (1.0 + sqrt(1.0 + epscorr * par.Thi_sin[Thi] * par.Thi_sin[Thi] ) ), 
                        par.zeta[Thi]);
        rad_mod[Thi] = ipow_vdw(fcrad[Thi], par.zeta[Thi]-1) / mod_norm[Thi];
      }
      // Loop over all bins
      for(int Rsi=0; Rsi<par.RsN_ang; Rsi++){
        for(int Thi=0; Thi<par.ThetasN; Thi++){
...
double PairPANNAvdw::compute_network(double *G, double *dEdG, int type){
  // *1 layer input
  // *2 layer output
  ...
  for(int l=0; l<par.Nlayers[type]; l++){
    ...
    matmul_vdw(dlay2,network[type][2*l],dlay1,size2,par.gsize,size1);
...
double PairPANNAvdw::switch_func(double xx)
{
  double S;
  double xx2 = xx * xx;
  double xx3 = xx2 * xx;
  ...
  double xx5 = xx4 * xx;
  if (xx > 1.0) S = 0.0;
  if (xx >= 0 && xx <= 1.0) S = -6.0 * xx5 + 15.0 * xx4 - 10.0 * xx3 + 1.0;  
  if (xx < 0) S = 1.0;
  return S;
}

double PairPANNAvdw::dswitch_func(double xx)
{
  double dS;
  double xx2 = xx * xx;
  double xx3 = xx2 * xx;
  double xx4 = xx3 * xx;
  if (xx > 1 || xx < 0) dS = 0.0;
  if (xx >= 0 && xx <= 1.0) dS = -30.0 * xx4 + 60.0 * xx3 - 30.0 * xx2;  
  return dS;
}
/**********************************************************/
//compute vdw contribution
/* ---------------------------------------------------------------------- */

void PairPANNAvdw::compute_vdw(double *Evdw)
{
  ...
  double Su, Sd, dSu, dSd, dvij, r6_inv, C6ij;
  ...
  double dxdr_u = 1.0 / (par.rmax_u - par.rmin_u);
  double dxdr_d = 1.0 / (par.rmax_d - par.rmin_d);
...
//    std::cout<<" inum  "<<i<<std::endl;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      r = sqrt(rsq);
      if (r > par.rmax_d) continue;

      double xij_u = (r - par.rmin_u) / (par.rmax_u - par.rmin_u);
      double xij_d = (r - par.rmin_d) / (par.rmax_d - par.rmin_d);
      Sd = switch_func(xij_d);
      Su = 1.0 - switch_func(xij_u);
      dSd = dswitch_func(xij_d);
      dSu = -dswitch_func(xij_u);
      
      C6ij = sqrt(par.species_C6[itype-1] * par.species_C6[jtype-1]);
      r6_inv = 1.0 / (rsq * rsq * rsq);
      
      //Evdw[ii] -= 0.5 *  C6ij * r6_inv * Su * Sd;
      Evdw[ii] -= C6ij * r6_inv * Su * Sd;

      
      dvij = C6ij * r6_inv * (6 * Su * Sd / r - dSu * Sd * dxdr_u - Su * dSd * dxdr_d);
      f[i][0] -= dvij * delx / r;
      f[i][1] -= dvij * dely / r;
      f[i][2] -= dvij * delz / r;
...
void PairPANNAvdw::compute(int eflag, int vflag)
{
  ...
  for(int a=0; a<inum; a++){
    ...
    if (eflag_global) eng_vdwl += Etmp[a] + Evdw[a];
    if (eflag_atom) eatom[myind] += Etmp[a] + Evdw[a];
  }
...
int PairPANNAvdw::get_input_line(std::ifstream* file, std::string* key, std::string* value){
  std::string line;
...
int PairPANNAvdw::get_parameters(char* directory, char* filename)
{
  ...
  while(parseint>0){
    ...
    if(parseint==1){
      ...
      // vdw section
      // If vdw section is found, change section
      else if(value=="LONG_RANGE_vdw"){
	...
	std::string comma = ",";
      ...
      // Parse species network
        ...
        if(key=="species_C6"){
          std::string comma = ",";
          size_t pos = 0; int s = 0;
          par.species_C6 = new double[par.Nspecies];
          value=value.substr(1, value.size() - 2); //get rid of [ ]
          while ((pos = value.find(comma)) != std::string::npos) {
            par.species_C6[s] = std::atof(value.substr(0, pos).c_str());
            value.erase(0, pos+1);  s++; }
          if(value.length()>0){ par.species_C6[s] = std::atof(value.c_str()); s++; };}
      
        if(key=="rmin_u"){
	  par.rmin_u = std::atof(value.c_str()); parset[3] = 1;
	}
        if(key=="rmax_u"){
	  par.rmax_u = std::atof(value.c_str()); parset[3] = 1;
	}
        if(key=="rmin_d"){
	  par.rmin_d = std::atof(value.c_str()); parset[3] = 1;
	}
        if(key=="rmax_d"){
	  par.rmax_d = std::atof(value.c_str()); parset[3] = 1;
	}
      }
      else{
        return -3;
  ...
  double cutmaxg = par.Rc_rad>par.Rc_ang ? par.Rc_rad : par.Rc_ang;
  par.cutmax = cutmaxg>par.rmax_d ? cutmaxg : par.rmax_d;
...
void PairPANNAvdw::coeff(int narg, char **arg)
{
...
double PairPANNAvdw::init_one(int i, int j)
{
...
void PairPANNAvdw::write_restart(FILE *fp)
{
...
void PairPANNAvdw::read_restart(FILE *fp)
{
...
void PairPANNAvdw::write_restart_settings(FILE *fp)
{
...
void PairPANNAvdw::read_restart_settings(FILE *fp)
{
...
// Not implemented.
void PairPANNAvdw::write_data(FILE *fp)
{

}

// Not implemented.
void PairPANNAvdw::write_data_all(FILE *fp)
{

}

// Not implemented.
double PairPANNAvdw::single(int i, int j, int itype, int jtype, double rsq,
...
void PairPANNAvdw::settings(int narg, char* argv[])
{
  if (narg != 0) {
