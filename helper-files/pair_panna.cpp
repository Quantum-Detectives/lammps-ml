...
void PairPANNA::compute(int eflag, int vflag)
{
  ...
  // We need to use cvflag_atom flag to specify all the components since the matrix is not symmetrix
  // for the virial contribution to the heat flux.
  // The associated variable should be cvatom
  ...
  for(int a=0; a<inum; a++){
    ...
    for(int n=0; n<nn; n++){
      ...
      // This is only needed to compute the virial contribution to heat current
//      std::cout << "cat_flag " << cvflag_atom << " xx done!" << std::endl;
      if (cvflag_atom){
	double dx = (x[myind][0] - x[nind][0]);
	double dy = (x[myind][1] - x[nind][1]);
	double dz = (x[myind][2] - x[nind][2]);

        cvatom[nind][0] += dx * ftmp[tshift + 3*n   ];
//	std::cout << "I am doing atomic stress " << cvatom[nind][0] << " xx done!" << std::endl;
        cvatom[nind][1] += dy * ftmp[tshift + 3*n + 1];
        cvatom[nind][2] += dz * ftmp[tshift + 3*n + 2];
        cvatom[nind][3] += dx * ftmp[tshift + 3*n + 1];
        cvatom[nind][4] += dx * ftmp[tshift + 3*n + 2];
        cvatom[nind][5] += dy * ftmp[tshift + 3*n + 2];
        cvatom[nind][6] += dy * ftmp[tshift + 3*n];
        cvatom[nind][7] += dz * ftmp[tshift + 3*n];
        cvatom[nind][8] += dz * ftmp[tshift + 3*n + 1];
      }
    }
